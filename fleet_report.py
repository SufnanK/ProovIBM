...
def car_wear(car: dict[str, object]) -> float:
    """Return service wear, or zero when no service reading is available."""
...
def fleet_summary(fleet: list[dict[str, object]]) -> dict[str, int | float]:
    """Summarize fleet size, service count, and average wear."""
    if not fleet:
...
def print_report(fleet: list[dict[str, object]]) -> None:
    ...
    distance = fleet_utils.format_number(fleet_utils.km_to_miles(total_km))
    print(f"Fleet distance: {distance} miles")
